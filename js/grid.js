/**
 * grid.js — Blast Battles 5×5 grid geometry
 ...
 *   20 21 22 23 24
 *
 * Player starts at 0 (top-left), bot starts at 24 (bottom-right).
 */
...
const ADJACENCY = (function () {
  ...
  for (let r = 0; r < 5; r++) {
    for (let c = 0; c < 5; c++) {
      const i = r * 5 + c;
      adj[i] = [];
      if (r > 0) adj[i].push((r - 1) * 5 + c); // up
      if (r < 4) adj[i].push((r + 1) * 5 + c); // down
      if (c > 0) adj[i].push(r * 5 + c - 1);   // left
      if (c < 4) adj[i].push(r * 5 + c + 1);   // right
    }
  }
  return adj;
})();
...
function rowOf(idx) { return Math.floor(idx / 5); }

/** Returns the 0-based column of a grid index. */
function colOf(idx) { return idx % 5; }
 ...
 *
 * @param {number} pos   - Origin tile index (0–24)
 ...
 * @param {number} pos   - Current tile index (0–24)
