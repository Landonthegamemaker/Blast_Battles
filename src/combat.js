 ...
 *   applyRangeMultiplier(baseDmg, card, dist)          → int
 *   applyPlayerWeaponBuff(dmg, card)                   → int
 *   applyBotWeaponBuff(dmg, card)                      → int
 *   applyLocationDamageBuff(dmg, char, pos, card?)     → int
 *   applyBotArmor(dmg, attackingCard)                  → { finalDmg, armorNote }
 *   applyPlayerArmor(dmg, attackingCard)               → { finalDmg, armorNote }
 *   getEffectiveSpeed(char, hand, inPlay)              → int
 *   getMaxHandSize(char)                               → int
 *   playerTotalCards()                                 → int
 *   botTotalCards()                                    → int
 *   healPlayer(amount)
 *   healBot(amount)
...
const MAX_TURNS = 10;
const MAX_MATCH_TIME_SEC = 300; // 10 turns - 30s each

// ── Range & damage multipliers ───────────────────────────────────────────────

/**
 * Scales base damage by how far the shooter is from their maximum range.
 * Melee always deals full damage at contact (range  — no scaling).
...
function applyRangeMultiplier(baseDmg, card, dist) {
  ...
  return Math.round(baseDmg * (effectiveDist / card.range));
}
